...
<div class="overflow-x-auto overflow-y-auto h-[75%] mt-4">
    <div class="font-times mx-auto" id="cetak" style="width: 180mm">
        <div class="bg-white ml-2 mr-2 mb-2 mt-2">
            ...
            <p class="font-bold text-center">Laporan Prestasi Siswa Kelas XII</p>
             {#each JDarray as j}
             <div class="flex justify-center item-center mt-6">
                <img src={`/images/${j.foto}`} alt="foto" class="w-[400px] border border-black">
             </div>
             <h1 class="text-center font-semibold">{j.keterangan}</h1>
            {/each}
        </div>
    </div>
</div>
<div class="flex justify-end gap-2 mt-2 mr-2 text-[10px] font-semibold">
    <button on:click={()=> goto('/dashboard/kesiswaan/bukti_fisik/_cetak/A4.1')} class="px-2 py-1 rounded-md bg-white border-2 border-red-400 hover:bg-red-600 hover:text-white">Kembali</button>
</div>
