...
<div class="overflow-x-auto overflow-y-auto h-[75%] mt-4">
    <div class="font-times mx-auto" id="cetak" style="width: 180mm">
        <div class="bg-white ml-2 mr-2 mb-2 mt-2">
            ...
            <p class="font-bold text-center">Sertifikat Guru dan Tenaga Kependidikan Yang Mengikuti Kegiatan Penguatan Kapasitas Diluar Madrasah</p>
             {#each JDarray as j}
             <div class="flex justify-center item-center mt-6">
                <img src={`/images/${j.foto}`} alt="foto" class="w-[400px] border border-black">
             </div>
             <h1 class="text-center font-semibold">Nama Guru: {j.nama}</h1>
             <p class="text-center">{j.keterangan}</p>
            {/each}
        </div>
    </div>
</div>
