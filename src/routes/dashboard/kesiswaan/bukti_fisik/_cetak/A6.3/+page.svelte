<script lang="ts">
    ...
    function kerjakan(kode: string) {
        goto(`/dashboard/kesiswaan/bukti_fisik/_cetak/A6.3/${kode}`)
    }
...
<div class="overflow-x-auto overflow-y-auto py-4 px-4 font-semibold flex justify-between border rounded-md shadow bg-white ml-2 mr-2 mb-2 mt-2 max-h-[90%] overflow:rounded-r-md">
    <button on:click={()=> kerjakan('Workshop')} class="px-2 py-1 rounded-md bg-white border-2 border-blue-400 hover:bg-blue-600 hover:text-white w-[48%]">Workshop</button>
    <button on:click={()=> kerjakan('Mgmp')} class="px-2 py-1 rounded-md bg-white border-2 border-green-400 hover:bg-green-600 hover:text-white w-[48%]">KKG-MGMP</button>
</div>
