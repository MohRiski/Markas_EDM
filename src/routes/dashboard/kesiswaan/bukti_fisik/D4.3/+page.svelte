<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'D4.3 Rekapitulasi daftar guru yang menggunakan buku teks atau buku digital'
                );
                if (draft) {
    ...
    async function simpanSementara() {
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'D4.3 Rekapitulasi daftar guru yang menggunakan buku teks atau buku digital',
    ...
    <table class="min-w-[1000px]  w-full table-auto">
        <thead class="bg-gray-100 text-left text-[13px] font-semibold">
            <tr>
                ...
                <th colspan="2" class="border px-2 py-1">Jenis Buku Yang Digunakan</th>
            </tr>
            <tr>
                <th class="border px-2 py-2">Teks</th>
                <th class="border px-2 py-2">Digital</th>
            </tr>
        </thead>
        <tbody class="text-sm">
          {#each JDarray as j}
            <tr class="border-t hover:bg-gray-50">
                ...
                <td class="px-2 py-2 text-[12px]">
                    <select id="bulan" bind:value={j.teks} class="w-full h-6 border px-1 rounded">
                        <option value=""></option>
                        <option value="✓">✓</option>
                ...
                <td class="px-2 py-2 text-[12px]">
                    <select id="bulan" bind:value={j.digital} class="w-full h-6 border px-1 rounded">
                        <option value=""></option>
                        <option value="✓">✓</option>
                        <option value="X">X</option>
                    </select>
