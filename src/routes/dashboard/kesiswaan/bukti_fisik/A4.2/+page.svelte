<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'Bukti prestasi siswa dalam berbagai lomba tingkat sekolah hingga internasional'
                );
                if (draft) {
    ...
    async function simpanPermanent() {
        ...
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'Bukti prestasi siswa dalam berbagai lomba tingkat sekolah hingga internasional',
                    nilai: nilaiBaru
                }
            ]
        };
