<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'Laporan prestasi siswa dari guru kelas/wali kelas. (Kelas XII)'
                );
                if (draft) {
    ...
    async function simpanSementara(file: string | File | null, keterangan: string) {
        ...
        const dataBaru = { foto: uploadedFilename, keterangan };

        const nilaiBaru = JSON.stringify([...(JDarray ?? []), dataBaru]);

        const newJawaban = {
            atribut: 'Laporan prestasi siswa dari guru kelas/wali kelas. (Kelas XII)',
            nilai: nilaiBaru
        };
    ...
    async function updateData() {
        ...
        const dataBaru = { foto: updatedFile, keterangan: keteranganBaru };
    ...
    <table class="min-w-[1000px]  w-full table-auto">
        ...
        <tbody class="text-sm">
          ...
          <tr class="border-t hover:bg-gray-50">              
            <td class="px-2 py-2">
                <p class="text-[12px] mt-1">Gambar: {j.keterangan}</p>
            </td>
...
<div class="flex justify-end gap-2 mt-2 mr-2 text-[10px] font-semibold">
    <button on:click={()=> goto('/dashboard/kesiswaan/bukti_fisik/A4.1')} class="px-2 py-1 rounded-md bg-white border-2 border-red-400 hover:bg-red-600 hover:text-white">Kembali</button>
</div>
