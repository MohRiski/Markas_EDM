<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'Laporan prestasi siswa dari guru kelas/wali kelas. (Kelas XI)'
                );
                if (draft) {
    ...
    async function simpanSementara(file: string | File | null, keterangan: string) {
        ...
        const newJawaban = {
            atribut: 'Laporan prestasi siswa dari guru kelas/wali kelas. (Kelas XI)',
            nilai: nilaiBaru
        };
