<script lang="ts">
    ...
    async function handleFileChange(event: any, index: any) {
        const file = event.target.files[0];
        if (!file) return;
        JDarray[index].file = file;
    }
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'E2.3 Rekapitulasi daftar guru dan tenaga kependidikan'
                );
                if (draft) {
    ...
    async function simpanPermanent() {
        ...
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'E2.3 Rekapitulasi daftar guru dan tenaga kependidikan',
                    nilai: nilaiBaru
                }
            ]
        };
...
<div class="overflow-x-auto overflow-y-auto border rounded-md shadow bg-white ml-2 mr-2 mb-2 mt-2 max-h-[80%] overflow:rounded-r-md">
    <table class="min-w-[1000px]  w-full table-auto">
        <thead class="bg-gray-100 text-left text-[13px] font-semibold">
          <tr>
            <th class="px-4 py-2">File Bantuan Untuk Kegiatan Penguatan Kapasitas</th>
