<script lang="ts">
    ...
    async function uploadFileIfNeeded(file: any) {
        if (file instanceof File) {
            ...
            const res = await fetch('/api/input_bukti_fisik/full_data/gambar', {
                method: 'POST',
                body: formData
            });
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'E2.4 Sertifikat dan laporan guru dan tenaga kependidikan'
                );
                if (draft) {
    ...
    async function simpanPermanent() {
        ...
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'E2.4 Sertifikat dan laporan guru dan tenaga kependidikan',
                    nilai: nilaiBaru
                }
            ]
        };
    ...
    async function simpanSementara(file: string | File | null, nama: string, keterangan: string) {
        if (!file) return false;

        const uploadedFilename = await uploadFileIfNeeded(file);
        const dataBaru = { foto: uploadedFilename, nama, keterangan };
    ...
    async function updateData() {
        ...
        const dataBaru = { foto: updatedFile, nama: namaBaru, keterangan: keteranganBaru };
    ...
    let namaPreview: string = '';
    let keteranganPreview: string = '';

    function lihatData(item: JDItem) {
        if (typeof item.foto === 'string') {
            filePreview = item.foto;
        } else if (item.foto instanceof File) {
            filePreview = URL.createObjectURL(item.foto);
        } else {
            filePreview = '';
        }

        keteranganPreview = item.keterangan;
        namaPreview = item.nama;
        modal = 'view';
        showModal = true;
    }
...
<div class="flex justify-end mr-2 mt-2">
    <button on:click={() => { showModal = true; modal = 'tambah'; fileBaru = null; namaBaru = ''; keteranganBaru = ''; }} class="text-[12px] h-[20px] bg-white border-2 border-green-400  px-2 font-semibold rounded hover:bg-green-500 flex hover:text-white">
...
<div class="overflow-x-auto overflow-y-auto border rounded-md shadow bg-white ml-2 mr-2 mb-2 mt-2 max-h-[80%] overflow:rounded-r-md">
    <table class="min-w-[1000px]  w-full table-auto">
        <thead class="bg-gray-100 text-left text-[13px] font-semibold">
          <tr>
            <th class="px-4 py-2">Foto Sertifikat</th>
        ...
        <tbody class="text-sm">
          {#each JDarray as j, i}
            <tr class="border-t hover:bg-gray-50">              
                <td class="px-2 py-2">
                    <p class="text-[12px] mt-1">Gambar: {j.keterangan} ({j.nama})</p>
                </td>
...
<div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
    {#if modal === 'tambah'}
        <div class="bg-white p-4 rounded-lg shadow-md w-80">
            <h3 class="font-bold text-md mb-2">Tambah Gambar</h3>
            ...
            <input type="text" class="w-full border px-2 py-1 text-sm rounded-lg mt-2" placeholder="Masukkan Nama" bind:value={namaBaru}>
            ...
            <div class="flex justify-end gap-2 mt-4 text-[12px] font-semibold">
                ...
                <button
                    on:click={() => {
                        if (fileBaru) {
                        simpanSementara(fileBaru, namaBaru, keteranganBaru);
                        }
                    }}
    ...
    {:else if modal === 'view'}
        <div class="bg-white p-4 rounded-lg shadow-md w-80">
            <h3 class="font-bold text-md mb-2">Preview Gambar</h3>
            {#if isImage(filePreview)}
                <img src={`/images/${filePreview}`} alt="preview" class="max-w-full max-h-[300px] rounded border" />
            {:else}
                <a href={`/images/${filePreview}`} target="_blank" class="text-blue-600 underline text-[14px]">
                    Download File
                </a>
            {/if}
            <p class="mt-2 text-sm text-gray-700"><strong>Nama Guru:</strong> {namaPreview}</p>
    ...
    {:else}
        <div class="bg-white p-4 rounded-lg shadow-md w-80">
            <h3 class="font-bold text-md mb-2">Edit Gambar</h3>
            <input type="file" class="w-full h-[34px] border px-2 py-1 rounded-lg text-[12px] bg-white file:bg-green-600 file:text-white file:rounded-md" on:change={(e) => fileBaru = (e.target as HTMLInputElement).files?.[0] ?? null}>
