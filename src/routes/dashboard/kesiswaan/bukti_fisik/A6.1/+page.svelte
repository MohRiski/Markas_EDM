<script lang="ts">
    ...
    $: JDarray = JDarray.map(item => ({
        ...item,
        jumjar: item.x + item.xi + item.xii
    }));
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'Rekapitulasi daftar penugasan guru dan tenaga kependidikan'
                );
                if (draft) {
    ...
    async function simpanSementara() {
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'Rekapitulasi daftar penugasan guru dan tenaga kependidikan',
...
<div class="overflow-x-auto overflow-y-auto border rounded-md shadow bg-white ml-2 mr-2 mb-2 mt-2 max-h-[80%] overflow:rounded-r-md">
    <table class="min-w-[1000px]  w-full table-auto">
        <thead class="bg-gray-100 text-left text-[13px] font-semibold">
            <tr>
                <th rowspan="2" class="border px-2 py-1">Nama Guru/NIP</th>
                <th rowspan="2" class="border px-2 py-1">Gol Ruang</th>
                <th rowspan="2" class="border px-2 py-1">TMT</th>
                <th rowspan="2" class="border px-2 py-1">Jabatan/Tugas Mengajar</th>
                <th colspan="3" class="border px-2 py-1 w-[20%]">Distribusi Jam Per Kelas</th>
                <th rowspan="2" class="border px-2 py-1 w-[5%]">Jumlah Jam Mengajar</th>
            </tr>
            <tr>
                <th class="border px-2 py-1">X</th>
                <th class="border px-2 py-1">XI</th>
                <th class="border px-2 py-1">XII</th>
            </tr>
        </thead>
        <tbody class="text-sm">
          {#each JDarray as j}
            <tr class="border-t hover:bg-gray-50">
                ...
                <td class="px-2 py-2 text-[12px]">
                    <input type="text" min="0" id="jumlah" bind:value={j.golru} class="w-full h-6 border px-1 rounded" />
                </td>
                <td class="px-2 py-2 text-[12px]">
                    <input type="text" min="0" id="melanggar" bind:value={j.tmt} class="w-full h-6 border px-1 rounded" />
                </td>              
                <td class="px-2 py-2 text-[12px]">
                    <input type="text" min="0" id="melanggar" bind:value={j.jabatan} class="w-full h-6 border px-1 rounded" />
                </td>
                <td class="px-2 py-2 text-[12px]">
                    <input type="number" min="0" id="melanggar" bind:value={j.x} class="w-full h-6 border px-1 rounded" />
                </td>              
                <td class="px-2 py-2 text-[12px]">
                    <input type="number" min="0" id="melanggar" bind:value={j.xi} class="w-full h-6 border px-1 rounded" />
                </td>              
                <td class="px-2 py-2 text-[12px]">
                    <input type="number" min="0" id="melanggar" bind:value={j.xii} class="w-full h-6 border px-1 rounded" />
                </td>                         
                <td class="px-2 py-2 text-[12px]">
                    <input type="number" min="0" id="melanggar" bind:value={j.jumjar} class="w-full h-6 border px-1 rounded" />
                </td>                         
            </tr>
          {/each}
        </tbody>
