<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'A1.1 Rekapitulasi daftar kehadiran'
                );
                if (draft) {
    ...
    async function simpanPermanent() {
        ...
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'A1.1 Rekapitulasi daftar kehadiran',
                    nilai: nilaiBaru
                }
            ]
        };
    ...
    async function simpanSementara(file: string | File | null, kelas: string, keterangan: string) {
        if (!file) return false;

        const uploadedFilename = await uploadFileIfNeeded(file);
        const dataBaru = { file: uploadedFilename, kelas, keterangan };
    ...
    async function updateData() {
        ...
        const dataBaru = { file: updatedFile, kelas: kelasBaru, keterangan: keteranganBaru };
...
<div class="flex justify-end mr-2 mt-2">
    <button on:click={() => { showModal = true; modal = 'tambah'; fileBaru = null; kelasBaru = ''; keteranganBaru = ''; }} class="text-[12px] h-[20px] bg-white border-2 border-green-400  px-2 font-semibold rounded hover:bg-green-500 flex hover:text-white">
        ...
        <tbody class="text-sm">
            {#each JDarray as j, i}
              <tr class="border-t hover:bg-gray-50">              
                <td class="px-2 py-2">
                    <p class="text-[12px] mt-1">File: {j.keterangan} (Kelas {j.kelas})</p>
                </td>
...
<div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
    {#if modal === 'tambah'}
        <div class="bg-white p-4 rounded-lg shadow-md w-80">
            ...
            <select id="kelas" bind:value={kelasBaru} class="w-full h-6 border px-2 rounded-lg mt-2 text-[12px]">
            ...
            <div class="flex justify-end gap-2 mt-4 text-[12px] font-semibold">
                ...
                <button
                    on:click={() => {
                        if (fileBaru) {
                        simpanSementara(fileBaru, kelasBaru, keteranganBaru);
                        }
                    }}
    ...
    {:else if modal === 'view'}
        <div class="bg-white p-4 rounded-lg shadow-md w-80">
            ...
            {/if}
            <p class="mt-2 text-sm text-gray-700"><strong>Keterangan:</strong> {keteranganPreview} (Kelas {kelasPreview})</p>
