<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'E1.3 Tangkapan layar (Capture) hasil pengisian aplikasi EDM dan e-RKAM.'
                );
                if (draft) {
    ...
    async function simpanPermanent() {
        ...
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'E1.3 Tangkapan layar (Capture) hasil pengisian aplikasi EDM dan e-RKAM.',
                    nilai: nilaiBaru
                }
            ]
        };
