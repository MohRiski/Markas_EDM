<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'Rekapitulasi daftar keaktifan guru dan tendik dalam (MGMP)'
                );
                if (draft) {
    ...
    async function simpan() {
        const newJawaban = {
            atribut: 'Rekapitulasi daftar keaktifan guru dan tendik dalam (MGMP)',
        ...
        if (!id_input_bukti_fisik) {
            ...
            if (res.ok) {
                ...
                goto('/dashboard/kesiswaan/bukti_fisik/A6.3');
            } else {
                Swal.fire({
                    title: 'Gagal!',
...
<div class="mb-4">
    {#each filterData as b}
        <h2 class="text-md font-bold ml-2">{b.kode} {b.keterangan} (KKG-MGMP)</h2>
    {/each} 
</div>
    ...
    <table class="min-w-[1000px]  w-full table-auto">
        <thead class="bg-gray-100 text-left text-[13px] font-semibold">
            <tr>
                ...
                <th class="border px-2 py-1">Jenis Guru</th>
                <th class="border px-2 py-1">Kelas</th>
                <th class="border px-2 py-1">Mapel</th>
                <th colspan="2" class="border px-2 py-1 w-[10%]">Jumlah Pelaksanaan MGMP</th>
                <th colspan="2" class="border px-2 py-1 w-[10%]">Frekuiensi Keikutsertaan</th>
                <th class="border px-2 py-1 w-[8%]">Prosentase</th>
        ...
        <tbody class="text-sm">
          {#each JDarray as j}
            <tr class="border-t hover:bg-gray-50">
                ...
                <td class="px-2 py-2 text-[12px]">
                    <input type="text" min="0" id="jumlah" bind:value={j.jenis} class="w-full h-6 border px-1 rounded" />
                </td>
                <td class="px-2 py-2 text-[12px]">
                    <select id="bulan" bind:value={j.kelas} class="w-full h-6 border px-1 rounded">
                        <option value="">Kelas</option>
                        <option value="X">X</option>
                        <option value="XI">XI</option>
                        <option value="XII">XII</option>
                        <option value="X,XI">X,XI</option>
                        <option value="X,XII">X,XII</option>
                        <option value="XI,XII">XI,XII</option>
                        <option value="X,XI,XII">X,XI,XII</option>
                    </select>
                </td>  
                <td class="px-2 py-2 text-[12px]">
                    <input type="text" min="0" id="jumlah" bind:value={j.mapel} class="w-full h-6 border px-1 rounded" />
                </td>
                <td class="px-2 py-2 text-[12px]">
                    <input type="number" min="0" id="jumlah" bind:value={j.jumlah} class="w-full h-6 border px-1 rounded" />
                </td>                                                                  
                <td class="px-2 py-2 text-[12px]">
                    Kali
                </td>                                                                  
                <td class="px-2 py-2 text-[12px]">
                    <input type="number" min="0" id="jumlah" bind:value={j.frekuensi} class="w-full h-6 border px-1 rounded" />
                </td>                                                                  
                <td class="px-2 py-2 text-[12px]">
                    Kali
                </td>                                                                  
                <td class="px-2 py-2 text-[12px]">
                    <input type="number" disabled min="0" id="jumlah" bind:value={j.prosentase} class="w-full h-6 border px-1 rounded" />
                </td>                                                                  
                <td class="px-2 py-2 text-[12px]">
                    <input type="text" min="0" id="jumlah" bind:value={j.keterangan} class="w-full h-6 border px-1 rounded" />
                </td>                                                                  
            </tr>
          {/each}
        </tbody>
...
<div class="flex justify-end gap-2 mt-2 mr-2 text-[10px] font-semibold">
    <button on:click={()=> goto('/dashboard/kesiswaan/bukti_fisik/A6.3')} class="px-2 py-1 rounded-md bg-white border-2 border-red-400 hover:bg-red-600 hover:text-white">Kembali</button>
