<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'Rekapitulasi daftar keaktifan guru dan tendik dalam (Workshop)'
                );
                if (draft) {
    ...
    async function simpan() {
        const newJawaban = {
            atribut: 'Rekapitulasi daftar keaktifan guru dan tendik dalam (Workshop)',
...
<div class="mb-4">
    {#each filterData as b}
        <h2 class="text-md font-bold ml-2">{b.kode} {b.keterangan} (Workshop)</h2>
    {/each} 
</div>
...
<div class="overflow-x-auto overflow-y-auto border rounded-md shadow bg-white ml-2 mr-2 mb-2 mt-2 max-h-[80%] overflow:rounded-r-md">
    <table class="min-w-[1000px]  w-full table-auto">
        <thead class="bg-gray-100 text-left text-[13px] font-semibold">
            <tr>
                <th class="border px-2 py-1">Nama Guru/NIP</th>
                <th class="border px-2 py-1">Jenis Kegiatan</th>
                <th class="border px-2 py-1">Penyelenggara</th>
                <th class="border px-2 py-1">Pelaksanaan</th>
        ...
        <tbody class="text-sm">
          {#each JDarray as j}
            <tr class="border-t hover:bg-gray-50">
                ...
                <td class="px-2 py-2 text-[12px]">
                    <input type="text" min="0" id="melanggar" bind:value={j.penyelenggara} class="w-full h-6 border px-1 rounded" />
                </td>              
                <td class="px-2 py-2 text-[12px]">
                    <input type="text" min="0" id="melanggar" bind:value={j.pelaksanaan} class="w-full h-6 border px-1 rounded" />
                </td>                       
                <td class="px-2 py-2 text-[12px]">
                    <input type="text" min="0" id="jumlah" bind:value={j.waktu} class="w-full h-6 border px-1 rounded" />
                </td>
                <td class="px-2 py-2 text-[12px]">
                    <input type="text" min="0" id="melanggar" bind:value={j.hasil} class="w-full h-6 border px-1 rounded" />
                </td>              
                <td class="px-2 py-2 text-[12px]">
                    <input type="text" min="0" id="melanggar" bind:value={j.keterangan} class="w-full h-6 border px-1 rounded" />
                </td>                       
            </tr>
          {/each}
        </tbody>
