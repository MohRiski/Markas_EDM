<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'Rekapitulasi daftar kehadiran guru untuk kegiatan pembelajaran, pembimbingan, dan pelatihan'
                );
                if (draft) {
    ...
    async function simpanSementara() {
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'Rekapitulasi daftar kehadiran guru untuk kegiatan pembelajaran, pembimbingan, dan pelatihan',
        ...
        <tbody class="text-sm">
          {#each JDarray as j}
            <tr class="border-t hover:bg-gray-50">
                ...
                <td class="px-2 py-2 text-[12px]">
                    <input type="number" min="0" id="jumlah" bind:value={j.jumlahGuru} class="w-full h-6 border px-1 rounded" />
                </td>              
                <td class="px-2 py-2 text-[12px]">
                    <input type="number" min="0" id="jumlah" bind:value={j.wajibHadir} class="w-full h-6 border px-1 rounded" />
                </td>
                <td class="px-2 py-2 text-[12px]">
                    <input type="number" min="0" id="melanggar" bind:value={j.S} class="w-full h-6 border px-1 rounded" />
                </td>              
                <td class="px-2 py-2 text-[12px]">
                    <input type="number" min="0" id="melanggar" bind:value={j.I} class="w-full h-6 border px-1 rounded" />
                </td>              
                <td class="px-2 py-2 text-[12px]">
                    <input type="number" min="0" id="melanggar" bind:value={j.A} class="w-full h-6 border px-1 rounded" />
                </td>              
                <td class="px-2 py-2 text-[12px]">
                    <input type="number" min="0" id="melanggar" bind:value={j.jumlah} class="w-full h-6 border px-1 rounded" />
                </td>              
                <td class="px-2 py-2 text-[12px]">
                    <input type="number" min="0" disabled step="0.01" id="prosentase" bind:value={j.prosentase} class="w-full h-6 border px-1 rounded" />
                </td>
                <td class="px-2 py-2 text-[12px]">
                    <textarea id="keterangan" bind:value={j.keterangan} class="w-full h-8 border px-1 rounded"></textarea>
                </td>
            </tr>
          {/each}
        </tbody>
