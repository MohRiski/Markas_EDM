<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'D3.2 Tangkapan layar (Capture) tampilan perangkat lunak dan laman web.'
                );
                if (draft) {
    ...
    async function simpanPermanent() {
        ...
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'D3.2 Tangkapan layar (Capture) tampilan perangkat lunak dan laman web.',
                    nilai: nilaiBaru
                }
            ]
        };
