<script lang="ts">
    ...
    $: cards = [
        { title: 'Pengguna', value: pengguna.length, color: 'bg-blue-500', link: '/dashboard/admin/pengguna' },
        { title: 'Hak Akses', value: hak_akses.length, color: 'bg-cyan-500', link: '/dashboard/admin/hak_akses' },
        { title: 'Indikator', value: indikator.length, color: 'bg-yellow-400', link: '/dashboard/admin/indikator' },
        { title: 'Bukti Fisik', value: bukti_fisik.length, color: 'bg-red-500', link: '/dashboard/admin/bukti_fisik' }
    ];
</script>
...
{:else}
    <div class="m-4 grid grid-cols-2 lg:grid-cols-4 gap-4">
        {#each cards as card}
    ...
    <div class="lg:flex justify-between h-[70%] m-4">
        <div class="rounded-lg border shadow-lg h-full lg:w-[40%]">
            {#if chartData.length > 0}
                <p class="text-[14px] font-semibold text-center mt-2 mb-2">Input Bukti Fisik Tahun {currentYear}</p>
        ...
        <div class="mt-4 pl-2 pr-2 lg:mt-0 rounded-lg border shadow-lg h-full lg:w-[58%]">
            {#if chartData.length > 0}
                <p class="text-[14px] font-semibold text-center mt-2 mb-2">Perkembangan Input Bukti Fisik Per Bulan</p>
