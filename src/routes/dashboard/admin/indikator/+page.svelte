<script lang="ts">
    ...
    function cariData() {
        if (keyword.trim() === '') {
            filterData = indikator;
        } else {
            filterData = indikator.filter((item) =>
    ...
    async function modalEdit(id_indikator: number) {
        const response = await fetch(`/api/indikator/id?id=${id_indikator}`, {
            method: 'GET'
        });
        if (response.ok) {
            const data = await response.json();
            id = data.data.id_indikator;
    ...
    async function edit() {
        if (kodeEdit === "" || keteranganEdit === "") {
			return;
		}
        const response = await fetch (`/api/indikator?id=${id}`, {
            method: "PUT",
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify({
                kode: kodeEdit, keterangan: keteranganEdit
            })
        });
        if (response.ok) {
            Swal.fire({
            ...
            Swal.fire({
                title: "Gagal!",
                text: "Edit data gagal dilakukan kode sudah ada",
                icon: "error",
                timer: 3000,
                background: "#8B0000",
                color: "#ffffff",
    ...
    async function tambah() {
        ...
        if (response.ok) {
            ...
            Swal.fire({
                title: "Gagal!",
                text: "Data gagal ditambah kode sudah ada",
                icon: "error",
                timer: 3000,
                background: "#8B0000",
                color: "#ffffff",
    ...
    async function hapus(id_indikator: string) {
        ...
        if (result.isConfirmed) {
            const response = await fetch(`/api/indikator?id=${id_indikator}`, {
                method: 'DELETE'
            });
            if (response.ok) {
                await dataIndikator();
...
<div class="flex justify-between">
    <h2 class="text-lg font-bold mb-2 ml-4">Indikator</h2>
...
{:else}
    <div class="overflow-x-auto overflow-y-auto border rounded-md shadow bg-white ml-4 mr-4 mb-2 mt-2 max-h-[90%] overflow:rounded-r-md">
        ...
        <tbody class="text-sm">
            {#each filterData as i}
            <tr class="border-t hover:bg-gray-50">
                <td class="px-4 py-2">{i.kode}</td>
                <td class="px-4 py-2">{i.keterangan}</td>
                <td class="px-4 py-2 flex gap-2">
                <button title="Edit" on:click={() => modalEdit(i.id_indikator)} class="text-black hover:text-blue-600">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-square-pen-icon lucide-square-pen"><path d="M12 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.375 2.625a1 1 0 0 1 3 3l-9.013 9.014a2 2 0 0 1-.853.505l-2.873.84a.5.5 0 0 1-.62-.62l.84-2.873a2 2 0 0 1 .506-.852z"/></svg>
                </button>
                <button title="Hapus" on:click={() => hapus(i.id_indikator)} class="text-black hover:text-red-600">
