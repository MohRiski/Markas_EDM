<script lang="ts">
    ...
    function cariData() {
        if (keyword.trim() === '') {
            filterData = hak_akses;
        } else {
            filterData = hak_akses.filter((item) =>
    ...
    async function modalEdit(id_hak_akses: number) {
        const response = await fetch(`/api/hak_akses/id?id=${id_hak_akses}`, {
            method: 'GET'
        });
        if (response.ok) {
            const data = await response.json();
            id = data.data.id_hak_akses;
            namaEdit = data.data.nama;
            form = "Edit"
            showForm = true;
        }
    }
    ...
    async function edit() {
        ...
        const response = await fetch (`/api/hak_akses?id=${id}`, {
            method: "PUT",
    ...
    async function hapus(id_hak_akses: string) {
        ...
        if (result.isConfirmed) {
            const response = await fetch(`/api/hak_akses?id=${id_hak_akses}`, {
                method: 'DELETE'
            });
            if (response.ok) {
                await dataHakAkses();
...
<div class="flex justify-between">
    <h2 class="text-lg font-bold mb-2 ml-4">Hak Akses</h2>
...
{:else}
    <div class="overflow-x-auto overflow-y-auto border rounded-md shadow bg-white ml-4 mr-4 mb-2 mt-2 max-h-[90%] overflow:rounded-r-md">
        ...
        <tbody class="text-sm">
            {#each filterData as h}
            <tr class="border-t hover:bg-gray-50">
                <td class="px-4 py-2">{h.nama}</td>
                <td class="px-4 py-2 flex gap-2">
                <button title="Edit" on:click={() => modalEdit(h.id_hak_akses)} class="text-black hover:text-blue-600">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-square-pen-icon lucide-square-pen"><path d="M12 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.375 2.625a1 1 0 0 1 3 3l-9.013 9.014a2 2 0 0 1-.853.505l-2.873.84a.5.5 0 0 1-.62-.62l.84-2.873a2 2 0 0 1 .506-.852z"/></svg>
                </button>
                <button title="Hapus" on:click={() => hapus(h.id_hak_akses)} class="text-black hover:text-red-600">
    ...
    {:else}
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" transition:fade>
            ...
            <div class="space-y-2 p-2">
                <div>
                    <label class="block font-semibold text-[14px]">Nama</label>
                    <input type="text" bind:value={namaEdit} class="w-full h-[34px] border px-3 py-2 rounded-lg text-[12px]" required />
                </div>
            </div>
