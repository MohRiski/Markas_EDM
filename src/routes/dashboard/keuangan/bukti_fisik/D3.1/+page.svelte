<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'D3.1 Daftar inventaris media berbasis TIK yang tersedia di madrasah'
                );
                if (draft) {
    ...
    async function simpanSementara() {
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'D3.1 Daftar inventaris media berbasis TIK yang tersedia di madrasah',
...
<div class="overflow-x-auto overflow-y-auto border rounded-md shadow bg-white ml-2 mr-2 mb-2 mt-2 max-h-[80%] overflow:rounded-r-md">
    <table class="min-w-[1000px]  w-full table-auto">
        <thead class="bg-gray-100 text-left text-[13px] font-semibold">
            <tr>
                <th rowspan="2" class="border px-2 py-1">Nama Alat/Bahan</th>
                <th rowspan="2" class="border px-2 py-1">Jumlah</th>
                <th colspan="2" class="border px-2 py-1">Kodisi</th>
                <th rowspan="2" class="border px-2 py-1">Keterangan</th>
            </tr>
            <tr>
                <th class="border px-2 py-2">Baik</th>
                <th class="border px-2 py-2">Rusak</th>
            </tr>
        </thead>
        <tbody class="text-sm">
          {#each JDarray as j}
            <tr class="border-t hover:bg-gray-50">
                ...
                <td class="px-2 py-2 text-[12px]">
                    <select id="bulan" bind:value={j.baik} class="w-full h-6 border px-1 rounded">
                        <option value=""></option>
                        <option value="✓">✓</option>
                ...
                <td class="px-2 py-2 text-[12px]">
                    <select id="bulan" bind:value={j.rusak} class="w-full h-6 border px-1 rounded">
                        <option value=""></option>
                        <option value="✓">✓</option>
                        <option value="X">X</option>
                    </select>
