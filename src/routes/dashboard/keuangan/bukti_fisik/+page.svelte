<script lang="ts">
    ...
    async function dataHakAkses() {
        ...
        hak_akses = res.data;
        const filter = hak_akses.filter((item: any) => item.nama === 'Operator Keuangan');

        id_keuangan = filter[0].id_hak_akses;
    }
    ...
    async function dataBuktiFisik() {
        ...
        const filter = data.data.filter((item: any) => item.penugasan_operator.some((ibf: any) => ibf.id_hak_akses === id_keuangan));
    ...
    function kerjakan(kode: string) {
        goto(`/dashboard/keuangan/bukti_fisik/${kode}`)
    }

    function cetak(kode: string) {
        goto(`/dashboard/keuangan/bukti_fisik/_cetak/${kode}`)
    }
