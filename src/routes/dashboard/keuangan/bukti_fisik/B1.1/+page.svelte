<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'Daftar undangan pelatihan/workshop/ pertemuan'
                );
                if (draft) {
    ...
    async function simpanSementara() {
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'Daftar undangan pelatihan/workshop/ pertemuan',
    ...
    <table class="min-w-[1000px]  w-full table-auto">
        <thead class="bg-gray-100 text-left text-[13px] font-semibold">
            <tr>
                ...
                <th colspan="3" class="border px-2 py-1 w-[50%]">Jenis PKB Yang Aktif Diikuti Guru</th>
                <th colspan="2" class="border px-2 py-1 w-[20%]">Keterangan</th>
            </tr>
            <tr>
                <th class="border px-2 py-1">Pengembangan Diri</th>
                <th class="border px-2 py-1">Publikasi Ilmiah</th>
                <th class="border px-2 py-1">Karya Inovasi</th>
        ...
        <tbody class="text-sm">
          {#each JDarray as j}
            <tr class="border-t hover:bg-gray-50">
                ...
                <td class="px-2 py-2 text-[12px]">
                    <select id="bulan" bind:value={j.pengembangan} class="w-full h-6 border px-1 rounded">
                        <option value=""></option>
                        <option value="✓">✓</option>
                ...
                <td class="px-2 py-2 text-[12px]">
                    <select id="bulan" bind:value={j.publikasi} class="w-full h-6 border px-1 rounded">
                        <option value=""></option>
                        <option value="✓">✓</option>
                        <option value="X">X</option>
                    </select>
                ...
                <td class="px-2 py-2 text-[12px]">
                    <select id="bulan" bind:value={j.karya} class="w-full h-6 border px-1 rounded">
                        <option value=""></option>
                        <option value="✓">✓</option>
                        <option value="X">X</option>
                    </select>
                ...
                <td class="px-2 py-2 text-[12px]">
                    <select id="bulan" bind:value={j.ya} class="w-full h-6 border px-1 rounded">
                        <option value=""></option>
                        <option value="✓">✓</option>
                        <option value="X">X</option>
                    </select>
                ...
                <td class="px-2 py-2 text-[12px]">
                    <select id="bulan" bind:value={j.tidak} class="w-full h-6 border px-1 rounded">
                        <option value=""></option>
                        <option value="✓">✓</option>
                        <option value="X">X</option>
                    </select>
