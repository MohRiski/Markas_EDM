<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'Bukti keikutsertaan pelatihan/workshop/bintek dalam bentuk sertifikat.'
                );
                if (draft) {
    ...
    async function simpanPermanent() {
        ...
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'Bukti keikutsertaan pelatihan/workshop/bintek dalam bentuk sertifikat.',
                    nilai: nilaiBaru
                }
            ]
        };
