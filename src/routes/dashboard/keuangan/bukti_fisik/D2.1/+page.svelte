<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'D2.1 Daftar media/alat peraga/alat bantu proses pembelajaran'
                );
                if (draft) {
    ...
    async function simpanSementara() {
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'D2.1 Daftar media/alat peraga/alat bantu proses pembelajaran',
    ...
    <table class="min-w-[1000px]  w-full table-auto">
        <thead class="bg-gray-100 text-left text-[13px] font-semibold">
            <tr>
                ...
                <th class="border px-4 py-2">Pengampu Mapel/Kelas</th>
                <th class="border px-4 py-2">Jenis Alat Bantu/Media Yang Dipakai</th>
                <th class="border px-4 py-2">Cara Peroleh</th>
                <th class="border px-4 py-2">Banyaknya</th>
        ...
        <tbody class="text-sm">
          {#each JDarray as j}
            <tr class="border-t hover:bg-gray-50">
                ...
                <td class="px-4 py-2 text-[12px]">
                    <input type="text" min="0" id="jumlah" bind:value={j.pengampu} class="w-full h-6 border px-1 rounded" />
                </td>
                <td class="px-4 py-2 text-[12px]">
                    <input type="text" min="0" id="jumlah" bind:value={j.jenis} class="w-full h-6 border px-1 rounded" />
                </td>
                <td class="px-4 py-2 text-[12px]">
                    <input type="text" min="0" id="jumlah" bind:value={j.cara} class="w-full h-6 border px-1 rounded" />
                </td>                                                            
                <td class="px-4 py-2 text-[12px]">
                    <input type="number" min="0" id="jumlah" bind:value={j.banyak} class="w-full h-6 border px-1 rounded" />
                </td>
                <td class="px-4 py-2 text-[12px]">
