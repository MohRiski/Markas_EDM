<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'Sertifikat keikutsertaan keikutsertaan kegiatan lokal/nasional/internasional'
                );
                if (draft) {
    ...
    async function simpanPermanent() {
        ...
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'Sertifikat keikutsertaan keikutsertaan kegiatan lokal/nasional/internasional',
                    nilai: nilaiBaru
                }
            ]
        };
