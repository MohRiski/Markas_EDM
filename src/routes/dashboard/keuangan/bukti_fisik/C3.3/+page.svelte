<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'C3.3 Rekapitulasi daftar guru yang menggunakan media pembelajaran'
                );
                if (draft) {
    ...
    async function simpanSementara() {
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'C3.3 Rekapitulasi daftar guru yang menggunakan media pembelajaran',
    ...
    <table class="min-w-[1000px]  w-full table-auto">
        <thead class="bg-gray-100 text-left text-[13px] font-semibold">
            <tr>
                ...
                <th rowspan="2" class="border px-2 py-1">Pengampu Mapel/Kelas</th>
                ...
                <th colspan="3" class="border px-2 py-1">Hasil Temuan Telah RPP</th>
            </tr>
            <tr>
                <th class="border px-2 py-2">1</th>
                <th class="border px-2 py-2">2</th>
                <th class="border px-2 py-2">Metode</th>
                <th class="border px-2 py-2">Media</th>
                <th class="border px-2 py-2">Penilaian</th>
            </tr>
        </thead>
        <tbody class="text-sm">
          {#each JDarray as j}
            <tr class="border-t hover:bg-gray-50">
                ...
                <td class="px-2 py-2 text-[12px]">
                    <select id="bulan" bind:value={j.metode} class="w-full h-6 border px-1 rounded">
                        <option value=""></option>
                        <option value="✓">✓</option>
                        <option value="X">X</option>
                    </select>
                ...
                <td class="px-2 py-2 text-[12px]">
                    <select id="bulan" bind:value={j.media} class="w-full h-6 border px-1 rounded">
                        <option value=""></option>
                        <option value="✓">✓</option>
                        <option value="X">X</option>
                    </select>
                ...
                <td class="px-2 py-2 text-[12px]">
                    <select id="bulan" bind:value={j.penilaian} class="w-full h-6 border px-1 rounded">
                        <option value=""></option>
                        <option value="✓">✓</option>
                        <option value="X">X</option>
                    </select>
