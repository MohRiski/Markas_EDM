<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'D1.2 Daftar buku bacaan dalam bentuk cetakan dan/atau digital di madrasah.'
                );
                if (draft) {
    ...
    async function simpanSementara() {
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'D1.2 Daftar buku bacaan dalam bentuk cetakan dan/atau digital di madrasah.',
