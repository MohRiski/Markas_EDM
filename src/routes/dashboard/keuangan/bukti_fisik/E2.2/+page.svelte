<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'E2.2 Laporan hasil wawancara TPM dengan guru dan tenaga kependidikan'
                );
                if (draft) {
    ...
    async function simpanSementara() {
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'E2.2 Laporan hasil wawancara TPM dengan guru dan tenaga kependidikan',
          ...
          {#each JDarray as j}
            <tr class="border-t hover:bg-gray-50">              
                <td class="px-2 py-2 text-[12px]">
                    <input type="text" id="keterangan" bind:value={j.narasumber} class="w-full h-8 border px-1 rounded">
                </td>
                <td class="px-2 py-2 text-[12px]">
                    <textarea id="keterangan" bind:value={j.pertanyaan} class="w-full h-8 border px-1 rounded"></textarea>
                </td>
                <td class="px-2 py-2 text-[12px]">
                    <textarea id="keterangan" bind:value={j.jawaban} class="w-full h-8 border px-1 rounded"></textarea>
                </td>
            </tr>
          {/each}
        </tbody>
