<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'D1.1 Daftar buku teks dalam bentuk cetakan dan/atau digital di madrasah.'
                );
                if (draft) {
    ...
    async function simpanSementara() {
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'D1.1 Daftar buku teks dalam bentuk cetakan dan/atau digital di madrasah.',
          ...
          {#each JDarray as j}
            <tr class="border-t hover:bg-gray-50">
                <td class="px-4 py-2 text-[12px]">
                    <input type="text" min="0" id="jumlah" bind:value={j.judul} class="w-full h-6 border px-1 rounded" />
                </td>
                <td class="px-4 py-2 text-[12px]">
                    <input type="text" min="0" id="jumlah" bind:value={j.pengarang} class="w-full h-6 border px-1 rounded" />
                </td>
                <td class="px-4 py-2 text-[12px]">
