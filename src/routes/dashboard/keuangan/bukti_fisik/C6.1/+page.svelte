<script lang="ts">
        ...
        if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                const draft = draftData.jawaban_bukti_fisik[0]?.detail_jawaban.find(
                    (d: { atribut: string; }) => d.atribut === 'C6.1 Dokumen perencanaan program remedial, pengayaan, pelayanan konseling'
                );
                if (draft) {
    ...
    async function simpanSementara() {
        const payload = {
            ...
            jawaban: [
                {
                    atribut: 'C6.1 Dokumen perencanaan program remedial, pengayaan, pelayanan konseling',
    ...
    <table class="min-w-[1000px]  w-full table-auto">
        <thead class="bg-gray-100 text-left text-[13px] font-semibold">
            <tr>
                ...
                <th rowspan="2" class="border px-2 py-1">Mapel</th>
                <th rowspan="2" class="border px-2 py-1">Kelas</th>
                <th colspan="3" class="border px-2 py-1">Program</th>
                <th rowspan="2" class="border px-2 py-1">Keterangan</th>
            </tr>
            <tr>
                <th class="border px-2 py-2">Remedial</th>
                <th class="border px-2 py-2">Pengayaan</th>
                <th class="border px-2 py-2">Konseling</th>
            </tr>
        </thead>
        <tbody class="text-sm">
          {#each JDarray as j}
            <tr class="border-t hover:bg-gray-50">
                ...
                <td class="px-2 py-2 text-[12px]">
                    <select id="bulan" bind:value={j.remedial} class="w-full h-6 border px-1 rounded">
                        <option value=""></option>
                        <option value="✓">✓</option>
                        <option value="X">X</option>
                    </select>
                ...
                <td class="px-2 py-2 text-[12px]">
                    <select id="bulan" bind:value={j.pengayaan} class="w-full h-6 border px-1 rounded">
                        <option value=""></option>
                        <option value="✓">✓</option>
                        <option value="X">X</option>
                    </select>
                ...
                <td class="px-2 py-2 text-[12px]">
                    <select id="bulan" bind:value={j.konseling} class="w-full h-6 border px-1 rounded">
                        <option value=""></option>
                        <option value="✓">✓</option>
                        <option value="X">X</option>
                    </select>
