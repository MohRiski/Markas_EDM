    ...
    <table class="min-w-[1000px]  w-full table-auto">
        <thead class="bg-gray-100 text-left text-[13px] font-semibold">
          <tr>
            ...
            <th class="px-4 py-2 w-[35%]">Kejadian/Prilaku</th>
        ...
        <tbody class="text-sm">
          {#each JDarray as j}
            <tr class="border-t hover:bg-gray-50">             
                <td class="px-4 py-2 text-[12px]">{j.tanggal}</td>
                <td class="px-4 py-2 text-[12px]">{j.nama}</td>
                <td class="px-4 py-2 text-[12px]">{j.kejadian}</td>
                <td class="px-4 py-2 text-[12px]">{j.sikap}</td>      
                <td class="px-4 py-2 text-[12px]">{j.nilai}</td>
            </tr>
          {/each}
        </tbody>
    </table> 
...
<div class="flex justify-end gap-2 mt-2 mr-2 text-[10px] font-semibold">
    <button on:click={()=> goto('/dashboard/kepala/bukti_fisik/A1.2')} class="px-2 py-1 rounded-md bg-white border-2 border-red-400 hover:bg-red-600 hover:text-white">Kembali</button>
</div>
